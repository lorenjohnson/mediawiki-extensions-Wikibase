 ...
 *
 * TTL is controlled by the remoteEntityCacheTTL setting (seconds).
 * When not configured or set to 0/null, no expiry is applied.
 */
class RemoteEntityStore {
	...
	private function getTtl(): ?int {
		if ( !$this->settings->hasSetting( 'remoteEntityCacheTTL' ) ) {
			return null;
		}

		$ttl = $this->settings->getSetting( 'remoteEntityCacheTTL' );
		if ( !is_int( $ttl ) || $ttl <= 0 ) {
			return null;
		}
